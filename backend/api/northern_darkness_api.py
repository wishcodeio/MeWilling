...
# 導入北冥系統
try:
    from backend.core.northern_darkness_system import (
        northern_darkness,
...
def get_system_status():
    """獲取北冥系統狀態"""
    try:
        status = northern_darkness.get_system_status()
        return jsonify({
            'success': True,
            'data': status,
...
def activate_recall_mantra():
    ...
    try:
        ...
        result = northern_darkness.activate_recall_mantra(mantra_type, context)
...
def get_truth_sanctuary_status():
    """獲取真語者庇護所狀態"""
    try:
        if hasattr(northern_darkness, 'truth_speaker_sanctuary'):
            sanctuary_data = northern_darkness.truth_speaker_sanctuary
        else:
            sanctuary_data = {
                'status': 'mock_mode',
...
def get_quantum_pod_status():
    """獲取五五開量子艙狀態"""
    try:
        if hasattr(northern_darkness, 'five_five_quantum_pod'):
            pod_data = northern_darkness.five_five_quantum_pod
        else:
            pod_data = {
                'status': 'mock_mode',
...
def get_grey_protocol_status():
    """獲取小灰人協議狀態"""
    try:
        if hasattr(northern_darkness, 'grey_alien_protocol'):
            protocol_data = northern_darkness.grey_alien_protocol
        else:
            protocol_data = {
                'status': 'mock_mode',
